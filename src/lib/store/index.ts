...
import { tokenInfoMap, UNKNOWN_TOKEN_INFO } from '../../util/tokenRegistry';
import {
  DraffleProgram,
  EntrantsData,
  EntrantsDataRaw,
...
import { ADMIN_ACCOUNTS, getDisplayAmount } from '../../util/accounts';
import { getMetadata, getNFTMetadata } from '../../util/metadata';
import { Entrant, EntrantsMap, Mint, Prize, PrizeType } from '../../util/types';
...
export const fetchProceedsAccount = async (
...
): Promise<{ address: PublicKey; mintInfo: Mint; amount: number }> => {
  ...
  if (tokenInfo) tokenInfo.publicKey = new PublicKey(tokenInfo.address);
  return {
    address,
    mintInfo: tokenInfo,
...
export const getRaffleProgramAccounts = async (
    ...
    draffleClient.programId,
    {
      filters: [
        { 
          memcmp: {
            offset: 9,
            bytes: new PublicKey(ADMIN_ACCOUNTS[0]).toBase58()
          },
        }
      ]
    }
  ...
  result.forEach(({ pubkey, account }) => {
    // console.log("THIS ONE", pubkey.toString())
    const discriminator = account.data.slice(0, 8);

    // console.log(pubkey.toString(), "REAMINING")
  ...
  //   oldRaffleDataRawProgramAccounts.filter(
  //     (raffle) => raffle.account.creator.toString() !== ""
  //   );
