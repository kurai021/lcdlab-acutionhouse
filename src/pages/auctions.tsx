...
function Auctions() {
    ...
    React.useEffect(() => {
        (async () => {
            let auctionFetch = await fetch("/api/auctions");
    ...
    return (
      <>
        ...
        <Container fluid className='containerAuctions'>
          {auctions.length === 0 && (
            <Row className='alertContainer'>
                <Col lg={{ span: 8, offset: 2 }}>
                  <Alert key='info' variant='info'>
                    <Alert.Heading>No auctions to show</Alert.Heading>
          ...
          {auctions.length >= 0 && (
            ...
            {auctions !== undefined &&
              ...
              auctions.map((auction: any) => (
                <AuctionCard handleRefresh={forceRefresh} auction={auction} />
              ))}
          </Row>
          )}
        </Container>
