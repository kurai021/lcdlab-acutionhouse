...
export default async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  if(type === "auction") {
      ...
      try {
        ...
        let result = await supabase.from('auctions').update({"hide": true}).eq("id", id)
  ...
  if(type === "market") {
      try {
        // console.log("inside", auction)
        let result = await supabase.from('market').update({"hide": true}).eq("id", id)
        console.log(result, "Successful")
        return res.send({
          message: true,
  ...
  if(type === "merch") {
      try {
        // console.log("inside", auction)
        let result = await supabase.from('merch').update({"hide": true}).eq("id", id)
        console.log(result, "Successful")
        return res.send({
          message: true,
  ...
  if(type === "raffle") {
      try {
        // console.log("inside", auction)
        let result = await supabase.from('raffles').update({"deleted": true}).eq("public_key", mintId)
        console.log(result, "Successful")
        return res.send({
          message: true,
