...
export default async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  const {mintId, auctionId, token}: any = req.query;
  const { data: auctions }: any = await supabase.from('auctionBids').select('*').match({"mint_id": mintId, "token": token});
  let newAuctions: any = await Promise.all(auctions.map(async (auction: any) => {
      const {created_at, id, mint_id, buyer, bid_amount} = auction;
      return {created_at, id, mint_id, buyer, bid_amount}
  }))
  return res.send({
    message: true,
    data: newAuctions,
