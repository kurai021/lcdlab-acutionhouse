...
export default async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  const { data: auctions }: any = await supabase.from('market').select('*')
  let newAuctions: any = await Promise.all(auctions.map(async (auction: any) => {
      const {quantity, SOL, created_at, end_at, id, mint_id, featured, starting_price, reserve_price, seller, hide, type, metadata, usd_cost} = auction;

        return {quantity, SOL, created_at, end_at, id, mint_id, seller, name: metadata.json.name, image: metadata.json.image, description: metadata.json.description, hide, type, featured, starting_price, reserve_price, usd_cost}
  }))
  return res.send({
    message: true,
    data: newAuctions,
