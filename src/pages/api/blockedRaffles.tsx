...
export default async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  const { data: auctions }: any = await supabase.from('raffles').select('*')
  let newAuctions: any = await Promise.all(auctions.map(async (auction: any) => {
      const {public_key, name, image, hide, deleted} = auction;
      if(deleted === true) {
        return [public_key, {name: name, overviewImageUri: image, hidden: hide}]
      }
      else return ["public_key",{name: "", overviewImageUri: "", hidden: true, }]
  }))
