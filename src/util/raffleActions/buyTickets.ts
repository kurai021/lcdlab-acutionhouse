...
import { VAULT_TOKEN_IN, VAULT_TOKEN_OUT } from '../accounts';
import { DISPENSER_REGISTRY_ADDRESS } from '..//programIds';
import { wrappedSOL } from '../tokenRegistry';
import {
  DispenserProgram,
  RaffleProgram,
...
export const buyTickets = async (
  ...
  if (paymentOption.mint.publicKey.toBase58() === wrappedSOL) {
    ...
    instructions.push(
      SystemProgram.transfer({
        fromPubkey: raffleClient.provider.wallet.publicKey,
  ...
  instructions.push(
    raffleClient.instruction.buyTickets(ticketAmount, {
      accounts: {
        raffle: raffle.publicKey,
        entrants: raffle.entrantsAccountAddress,
        proceeds: raffle.proceeds.address,
        buyerWallet: raffleClient.provider.wallet.publicKey,
        buyerTokenAccount: finalBuyerAccount,
        buyerTransferAuthority: raffleClient.provider.wallet.publicKey,
        raffleWallet: new PublicKey("bRafUHPMteqcHuC8pG9CSDShbEqcfgYatxpTru1bcoK"),
        tokenProgram: TOKEN_PROGRAM_ID,
